function displayRepositories(event) {
  ...
  const repoList = `<ul>${repos.map(r => '<li>' + `<a target="_blank" href="${r.html_url}">${r.name}</a>` + ' - <a href="#" data-repository="' + r.name + '" data-username="' + r.owner.login + '" onclick="getCommits(this)">Get Commits</a></li>').join('')}</ul>`;
